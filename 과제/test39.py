...
print([[word for word in inputWord if ord(m[0]) <= ord(word[0]) <= ord(m[1])] for m in dicBase])

# 아래와 같이 가나다로 묶은 상태에서 정렬을 해줘도 좋을 것 같다.
print([list(sorted(word for word in inputWord if ord(m[0]) <= ord(word[0]) <= ord(m[1]))) for m in dicBase])
